...
var opts = {
  ...
  upIn: [
    ...
    animate(styles.inOutQuadratic, style({ transform: '*' }))
  ],
  upOut: [
    style({ transform: '*' }),
    animate(styles.inOutQuadratic, style({ transform: 'translateY(100%)' }))
  ],
  zoomIn: [
...
export const upIn = [trigger('upIn', [transition('void => *', opts.upIn)])]
export const upOut = [trigger('upOut', [transition('* => void', opts.upOut)])]
...
export const simAnim = [
  trigger('simAnim', [
    ...
    transition('* => upIn', opts.upIn),
    transition('* => upOut', opts.upOut),
